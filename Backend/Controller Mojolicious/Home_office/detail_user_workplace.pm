package Controller::detail_user_workplace;
...
# Render data specific to the user's workplace to detail_user_workplace.html.ep
sub render_user_workplace_data {
    my ($self, $user_id) = @_;
    ...
    # Fetch workplace data for the user based on user_id
    my $workplace_p = $ua->get_p("$workplace_url?user_id=$user_id")->then(sub {
        my $result = shift->result;
        return $result->is_success ? $result->json : die "Workplace API error: " . $result->message;
    });

    # Fetch devices associated with the user's workplace
    ...
    Mojo::Promise->all($user_p, $workplace_p, $devices_p)->then(sub {
        my ($user_res, $workplace_res, $devices_res) = @_;

        # Render the detail_user_workplace.html.ep template with data from all entities
        $self->render(
            template  => 'detail_user_workplace',
            user      => $user_res->[0],
